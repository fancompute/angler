...
def scan_power(simulation, probes=None, omega=None, Ns=10, s_min=1e-1, s_max=1e3, solver='newton'):
...
def scan_frequency(D, probes, Nf=40, df=1/200, fmin=None, fmax=None, pbar=True):
    ...
    for fi, f in enumerate(freqs):
        ...
        for pi, p in enumerate(probes):
            spectra[pi].append(Ts_up[pi])
            spectra[pi+1].append(Ts_down[pi])
...
if __name__ == '__main__':
    ...
    # np.save('data/spectra/freqs2_bi', freqs)
    # np.save('data/spectra/spectra2_bi', spectra2)
